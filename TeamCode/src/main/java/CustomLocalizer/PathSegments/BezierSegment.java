...
public class BezierSegment extends PathSegment {
    ...
    public BezierSegment(Pose2D startPos, Pose2D endPos, double anchorLength, Telemetry telemetry) {
        ...
        p2 = startPos.getVector().add(Vector2.fromAngle(Math.toRadians(startPos.heading - 90)).multiply(anchorLength));
        p4 = endPos.getVector();
        p3 = endPos.getVector().subtract(Vector2.fromAngle(Math.toRadians(endPos.heading - 90)).multiply(anchorLength));
